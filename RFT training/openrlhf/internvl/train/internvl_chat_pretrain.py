...
class LazySupervisedDataset(Dataset):
    ...
    def __iter__(self):
        ...
        if self.worker_id == 0:
            logger.info(f"[{self.ds_name}] [Worker id {self.worker_id}] " f"begin to iter with {start_idx=}")
...
def main():
    ...
    if model_args.model_name_or_path is not None:
        ...
        llm = model_type.from_pretrained(
            model_args.llm_path, torch_dtype=torch.bfloat16, config=llm_config, trust_remote_code=True
        )
